...
import {
  ...
  StatusBar, Dimensions, Image, ActivityIndicator,
} from 'react-native';
import Animated, {
  useSharedValue, useAnimatedStyle, withTiming, withDelay, interpolate, Extrapolation,
...
function RoomCard({ room, onPress, theme, currency }) {
  ...
  const imgUrl = room.images?.[0]?.image_path
    ? `https://hotel.primelogic.com.np/${room.images[0].image_path}`
    : null;
  return (
    <AnimatedCard onPress={onPress} style={styles.roomCard}>
      <View style={styles.roomImageBox}>
        {imgUrl
          ? <Image source={{ uri: imgUrl }} style={styles.roomImage} />
        ...
        <View style={styles.roomPriceBadge}>
          ...
          <Text style={[styles.roomPriceNight, { color: theme.primary }]}>/night</Text>
        </View>
      </View>
    ...
    <View style={[styles.reviewCard, { backgroundColor: theme.white, borderColor: theme.border }]}>
      <View style={styles.reviewHeader}>
        <View style={[styles.reviewAvatar, { backgroundColor: theme.primary }]}>
          <Text style={[styles.reviewInitial, { color: theme.secondary }]}>
            {(review.guest_name || review.reviewer_name || 'G')[0].toUpperCase()}
          </Text>
        </View>
        <View style={styles.reviewMeta}>
          <Text style={[styles.reviewName, { color: theme.primary }]}>{review.guest_name || review.reviewer_name || 'Guest'}</Text>
          <StarRating rating={review.rating} />
        </View>
      </View>
      <Text style={[styles.reviewText, { color: theme.text }]} numberOfLines={3}>{review.review || review.comment}</Text>
    </View>
  );
}
...
function AmenityItem({ amenity, theme }) {
  return (
    <View style={[styles.amenityItem, { backgroundColor: theme.primary + '12', borderColor: theme.primary + '20' }]}>
      <Text style={styles.amenityIcon}>{amenity.icon || '✨'}</Text>
      <Text style={[styles.amenityName, { color: theme.primary }]} numberOfLines={1}>{amenity.name}</Text>
    </View>
  );
}
...
export default function HomeScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setReviews(revData.filter(r => r.is_approved).slice(0, 5));
      const amData = Array.isArray(am.data) ? am.data : (am.data?.data || []);
      setAmenities(amData.slice(0, 8));
        ...
        <View style={styles.body}>
          ...
          {amenities.length > 0 && (
            <>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.amenitiesRow}>
          ...
          <View style={[styles.contactCard, { backgroundColor: theme.primary }]}>
            <Text style={[styles.contactTitle, { color: theme.secondary }]}>Contact Us</Text>
...
const styles = StyleSheet.create({
  ...
  heroEyebrow: { fontSize: 13, color: 'rgba(255,255,255,0.7)', letterSpacing: 3, textTransform: 'uppercase', marginBottom: 8 },
  heroHotelName: { fontSize: 40, fontWeight: '800', color: '#fff', letterSpacing: -1, marginBottom: 12 },
  heroLine: { width: 50, height: 2, marginBottom: 12 },
  heroTagline: { fontSize: 16, color: 'rgba(255,255,255,0.8)', lineHeight: 24, marginBottom: 20 },
  currencyToggle: { marginLeft: -20, marginBottom: 20 },
  heroBtn: {
    alignSelf: 'flex-start', paddingHorizontal: 28, paddingVertical: 14,
    borderRadius: 999,
  ...
  sectionLine: { width: 36, height: 2, marginTop: 4 },
  ...
  roomMeta: { flexDirection: 'row', gap: 12, marginBottom: 12 },
  ...
  amenitiesRow: { gap: 10, paddingBottom: 8 },
  amenityItem: {
    width: 90, alignItems: 'center', padding: 12,
    borderRadius: 14, borderWidth: 1,
  },
  amenityIcon: { fontSize: 24, marginBottom: 6 },
  amenityName: { fontSize: 10, fontWeight: '600', textAlign: 'center' },
  ...
  contactCard: { borderRadius: 20, padding: 24, marginTop: 8, marginBottom: 24 },
